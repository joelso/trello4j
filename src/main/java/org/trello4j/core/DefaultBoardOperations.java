...
public class DefaultBoardOperations extends AbstractOperations implements BoardOperations {
	...
	@Override
	public Board get() {
		TrelloURL uri = getTrelloAccessor().createTrelloUri(TrelloURL.BOARD_URL, boardId);
		return getTrelloAccessor().doGet(uri.build(), Board.class);
	}

	@Override
	public List<Action> getActions(final String... filters) {
		TrelloURL uri = getTrelloAccessor().createTrelloUri(TrelloURL.BOARD_ACTIONS_URL, boardId).filter(filters);
	...
	public List<Card> getCards(final String... filters) {
		TrelloURL uri = getTrelloAccessor().createTrelloUri(TrelloURL.BOARD_CARDS_URL, boardId).filter(filters);
	...
	public List<Checklist> getChecklist() {
		TrelloURL uri = getTrelloAccessor().createTrelloUri(TrelloURL.BOARD_CHECKLISTS_URL, boardId);
	...
	public List<org.trello4j.model.List> getList(final String... filters) {
		TrelloURL uri = getTrelloAccessor().createTrelloUri(TrelloURL.BOARD_LISTS_URL, boardId).filter(filters);
	...
	public List<Member> getMembers(final String... filters) {
		TrelloURL uri = getTrelloAccessor().createTrelloUri(TrelloURL.BOARD_MEMBERS_URL, boardId).filter(filters);
	...
	public List<Member> getInvitedMembers(final String... filters) {
		TrelloURL uri = getTrelloAccessor().createTrelloUri(TrelloURL.BOARD_MEMBERS_INVITED_URL, boardId).filter(filters);
		ParameterizedTypeReference<List<Member>> typeReference = new ParameterizedTypeReference<List<Member>>() {
		};
		return getTrelloAccessor().doGet(uri.build(), typeReference);
	}
	...
	public Prefs getPrefs() {
		TrelloURL uri = getTrelloAccessor().createTrelloUri(TrelloURL.BOARD_PREFS_URL, boardId);
		return getTrelloAccessor().doGet(uri.build(), Prefs.class);
	}

	@Override
	public Organization getOrganization(final String... filters) {
		TrelloURL uri = getTrelloAccessor().createTrelloUri(TrelloURL.BOARD_ORGANIZAION_URL, boardId).filter(filters);
